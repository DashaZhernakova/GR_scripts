...
setwd("/Users/dashazhernakova/Documents/Doby/GenomeRussia/freeze2019/population_genetics/finestructure/")
source("/Users/dashazhernakova/Documents/Doby/GenomeRussia/60samples/population_genetics/finestructure/scripts/FinestructureRcode/FinestructureLibrary.R") # read in the R functions, which also calls the needed packages
source("/Users/dashazhernakova/Documents/Doby/GenomeRussia/60samples/population_genetics/finestructure/scripts/FinestructureRcode/FinestructureDendrogram.R")
...
#chunkfile<-"GR+papers_all_chr_linked.chunkcounts.out" ## chromopainter chunkcounts file
mcmcfile<-"all_chr_gr+papers_linked_mcmc_run1.xml" ## finestructure mcmc file
treefile<-"all_chr_gr+papers_linked_tree_run1.xml" ## finestructure tree file

## Additional files that you can extract from finestructure
mappopchunkfile<-"GR+papers_all_chr_linked.mapstate.csv" # population-by-population chunkcount file for the populations used in the MAP (i.e tree)
#system( paste("fs fs -X -Y -e X2",chunkfile,treefile,mappopchunkfile) )
meancoincidencefile<-"GR+papers_all_chr_linked.meancoincidence.csv" # pairwise coincidence, .i.e. proportion of MCMC files where individuals are found in the same 
...
pdf(file="plots/FullDendrogram2.pdf",height=15,width=5)
par(mar=c(6,0,2,0),mfrow=c(1,1))
plot.dendrogram(tdend,horiz=TRUE,nodePar=list(cex=0,lab.cex=0.2),edgePar=list(p.lwd=0, t.srt=0,t.off=-0.5, t.cex = 0.1),axes=F)
dev.off()

pdf(file="plots/FullDendrogram2_weights.pdf",height=15,width=5)
par(mar=c(6,0,2,0),mfrow=c(1,1))
plot.dendrogram(tdend,horiz=TRUE,nodePar=list(cex=0,lab.cex=0.2),edgePar=list(p.lwd=0, t.srt=0,t.off=-0.5, t.cex = 0.5),axes=F)
dev.off()
...
pdf(file="plots//PopulationDendrogram.pdf",height=6,width=12)
...
#pdf(file="plots/PairwiseCoincidence.pdf",height=12,width=12)
...
pdf(file="plots/Coancestry.pdf",height=12,width=12)
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=0.3,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=0.8))
dev.off()
...
tmpmat[tmpmat>tmatmax]<-tmatmax # 
pdf(file="plots/EastAsiaPopAveragedCoancestry.pdf",height=12,width=12)
...
pdf(file="plots/Coancestry2.pdf",height=12,width=12)
...
pdf("plots/Example_PCA.pdf",height=16,width=12)
par(mfrow=c(4,3))
...
copyprobsfile<-"../GR+1000G_chr22_3.copyprobsperlocus.chunkcounts.out"
## file contains only haplotypes from individual 1 (it was run with "chromopainter -a 1 1 -b -in -iM -i 10 -g .chrom1.phase -r .chrom1.trecombfile -o .chrom1.linked.hap1")
