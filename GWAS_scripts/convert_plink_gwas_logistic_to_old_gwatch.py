...
def writeTests(test_list, outbase, module_name):
	...
	tests_dict = {}
	test_prefix = "CAT/" + module_name + "/HIV/"
		...
		for l in bim_f:
			...
			if cur_chr != chr:
				...
				try:
					...
					print "\t\tno files to close"
					pass
			
				snps_out_f = open(outbase + "chr" + chr + "_T2.csv", "w")
			...
			snp_dict[spl[0] + ":" + spl[3]] = str(snp_idx)
			#print snp_dict
			#if snpid == "rs3131968":
			#	break
			snp_idx += 1
...
def writeRes(res_fname, test, snp_dict, test_dict, outbase):
	print "Writing tests"
	test_name_translation = {'D' : 'DOM', 'R' : 'REC', 'A' : 'ADD', 'CD' : 'GENO_2DF'}
	...
	with open(res_fname) as res_f:
		for l in res_f:
			...
			if spl[8] != "NA":
				pval = spl[8]
			res_out_f.write(cur_chr + "," + snp_dict[spl[0] + ":" + spl[2]] + "," + test_dict[test] + "," + pval + "," + OR + "\n")
	res_out_f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o','--out', help='Output file name prefix', required=True)
