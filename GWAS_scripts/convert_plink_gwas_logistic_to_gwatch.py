	...
	with open(bim_fname) as bim_f:
		for l in bim_f:
			...
			if cur_chr != chr:
				...
				snps_out_f = open(outbase + "_chr" + chr + "_T2.csv", "w")
	...
	with open(res_fname) as res_f:
		for l in res_f:
			...
			if cur_chr != chr:
				...
				fname = outbase + "_chr" + chr + "_T3.csv"
				...
				cur_chr = chr
			res_out_f.write(cur_chr + "," + snp_dict[spl[1]] + "," + test_dict[test] + "," + spl[8] + "," + spl[6] + "\n")
	res_out_f.close()
